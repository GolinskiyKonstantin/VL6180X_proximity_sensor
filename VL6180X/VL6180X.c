...
inline static void VL6180x_getRegister(VL6180x* DataStruct, uint16_t registerAddr);
inline static void VL6180x_getRegister16bit(VL6180x* DataStruct, uint16_t registerAddr);
inline static void VL6180x_setRegister(VL6180x* DataStruct, uint16_t registerAddr, uint8_t data);
inline static void VL6180x_setRegister16bit(VL6180x* DataStruct, uint16_t registerAddr, uint16_t data);


/*
	******************************************************************************
	* @brief	 ( описание ):  	функция инициализации датчика
	* @param	( параметры ):		структура датчика, шина i2c, и адресс датчика
	* @return  ( возвращает ):	
...
void VL6180x_init(VL6180x* DataStruct, I2C_HandleTypeDef hi2c, uint8_t address) 
{
	...
	// конфиг регистров которых нет в даташите но есть в отдельном документе от ST
	// он всегда один и тотже менять нечего не нужно
	...
	// регистрі из даташита отвечают за настройки работі датчика
	
	// Recommended : Public registers - See data sheet for more detail
	VL6180x_setRegister(DataStruct, VL6180X_SYSTEM_MODE_GPIO1, 								0x10); 		// Enables polling for New Sample ready when measurement completes
	VL6180x_setRegister(DataStruct, VL6180X_READOUT_AVERAGING_SAMPLE_PERIOD, 	0x30);		// Set the averaging sample period (compromise between lower noise and increased execution time)
	VL6180x_setRegister(DataStruct, VL6180X_SYSALS_ANALOGUE_GAIN, 						0x46);		// Sets the light and dark gain (upper nibble). Dark gain should not be changed
	VL6180x_setRegister(DataStruct, VL6180X_SYSRANGE_VHV_REPEAT_RATE, 				0xFF);		// sets the # of range measurements after which auto calibration of system is performed
	VL6180x_setRegister(DataStruct, VL6180X_SYSALS_INTEGRATION_PERIOD, 				0x63);		// Set ALS integration time to 100ms
	VL6180x_setRegister(DataStruct, VL6180X_SYSRANGE_VHV_RECALIBRATE, 				0x01);		// perform a single temperature calibration of the ranging sensor

	// Optional: Public registers - See data sheet for more detail
	VL6180x_setRegister(DataStruct, VL6180X_SYSRANGE_INTERMEASUREMENT_PERIOD, 0x09); 		// Set default ranging inter-measurement period to 100ms
	VL6180x_setRegister(DataStruct, VL6180X_SYSALS_INTERMEASUREMENT_PERIOD, 	0x31); 		// Set default ALS inter-measurement period to 500ms
	VL6180x_setRegister(DataStruct, VL6180X_SYSTEM_INTERRUPT_CONFIG_GPIO, 		0x24); 		// Configures interrupt on New sample ready
...
/*
	******************************************************************************
	* @brief	 ( описание ):  	функция получения данных о датчике
	* @param	( параметры ):		структура датчика, структура для данных о датчике
	* @return  ( возвращает ):	

	******************************************************************************
*/
void VL6180x_getIdentification(VL6180x* DataStruct, VL6180xIdentification *temp)
{
...
/*
	******************************************************************************
	* @brief	 ( описание ):  	функция изменения адресса датчика ( после отключения питания адресс не сохраняеться )
	* @param	( параметры ):		структура датчика, новый адресс датчика
	* @return  ( возвращает ):	
...
/*
	******************************************************************************
	* @brief	 ( описание ):  	функция получения дистанции
	* @param	( параметры ):		структура датчика
	* @return  ( возвращает ):	

	******************************************************************************
*/
void VL6180x_getDistance(VL6180x* DataStruct) 
{
...
/*
	******************************************************************************
	* @brief	 ( описание ):  	функция получения освещенности
	* @param	( параметры ):		структура датчика, чувчтвительность ( смотри даташит )
	* @return  ( возвращает ):	

	******************************************************************************
*/
void VL6180x_getAmbientLight(VL6180x* DataStruct, vl6180x_als_gain VL6180X_ALS_GAIN)
{
  ...
  uint32_t alsRaw = DataStruct->data_16;
  ...
  uint32_t alsIntegrationPeriodRaw = DataStruct->data_16;
...
inline static void VL6180x_getRegister(VL6180x* DataStruct, uint16_t registerAddr)
{
  ...
  HAL_I2C_Master_Transmit(&DataStruct->hi2cx, DataStruct->address, data_write, 2, 1000);
  HAL_I2C_Master_Receive(&DataStruct->hi2cx, DataStruct->address, data_read, 1, 1000);
...
inline static void VL6180x_getRegister16bit(VL6180x* DataStruct, uint16_t registerAddr)
{
  ...
  HAL_I2C_Master_Transmit(&DataStruct->hi2cx, DataStruct->address, data_write, 2, 1000);
  HAL_I2C_Master_Receive(&DataStruct->hi2cx, DataStruct->address, data_read, 2, 1000);
...
inline static void VL6180x_setRegister(VL6180x* DataStruct, uint16_t registerAddr, uint8_t data)
{
    ...
    HAL_I2C_Master_Transmit(&DataStruct->hi2cx, DataStruct->address, data_write, 3, 1000);
}
...
inline static void VL6180x_setRegister16bit(VL6180x* DataStruct, uint16_t registerAddr, uint16_t data)
{
    ...
    HAL_I2C_Master_Transmit(&DataStruct->hi2cx, DataStruct->address, data_write, 4, 1000);
}
//----------------------------------------------------------------------------------
